...
class GoogleAnalytics
{
    public function __construct(modX &$modx, array $config = array())
    {
        ...
        $basePath = $this->modx->getOption(
            'analyticsdashboardwidget.core_path',
            $config,
            $this->modx->getOption('core_path').'components/analyticsdashboardwidget/'
        );
        $assetsUrl = $this->modx->getOption(
            'analyticsdashboardwidget.assets_url',
            $config,
            $this->modx->getOption('assets_url').'components/analyticsdashboardwidget/'
        );
        $this->config =array(
            'basePath' => $basePath,
            'corePath' => $basePath,
    ...
    public function parseData($xml)
    {
        ...
        foreach ($entries as $entry) {
            ...
            foreach ($dimensions as $dimension) {
                if ($dimension->getAttribute("name") == 'ga:isMobile' ||
                    $dimension->getAttribute("name") == 'ga:date'
                ) {
            ...
            foreach ($metrics as $metric) {
                if ($metric->getAttribute('name') == 'ga:avgTimeOnSite') {
                    $results[$i][ltrim($metric->getAttribute('name'), "ga:")] =
                        $this->secondMinute($metric->getAttribute('value'));
                } else {
